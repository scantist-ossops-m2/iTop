...
class CSVImport
{
	 ...
	 */
private function GetFunctionParametersAndValues($functionName, $args): array
	{
		$reflectionFunc = new ReflectionMethod($this, $functionName);
	 ...
	 * @throws Exception
	 */
    public static function _ProcessCSVData(
        mixed $iBoxSkipLines, mixed $iNbSkippedLines, mixed $sDateTimeFormat, mixed $sCustomDateTimeFormat, mixed $sSynchroScope, mixed $sClassName, WebPage $oPage, $aSynchroUpdate, mixed $sCSVData,
        mixed $sSeparator, mixed $sTextQualifier, bool $bHeaderLine, array $aResult, mixed $aSearchFields,
        mixed $aFieldsMapping, bool $bSimulate, mixed $sCSVDataTruncated, int $iCurrentStep, mixed $sEncoding, mixed $bAdvanced
    ): ?array
    {
		// TODO cleanup appel fonction + fonction
	    $args = func_get_args();
	    $paramValues = (new CSVImport())->GetFunctionParametersAndValues(__FUNCTION__, $args);
		unset($paramValues['oPage']);
		file_put_contents(__DIR__."/log.txt", var_export($paramValues, true));
